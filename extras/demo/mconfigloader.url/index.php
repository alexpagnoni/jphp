...
<tr>
   <td bgcolor=#ffffff>
   <strong>jphp://jphp.demo/MultiConfigLoader/<a href="<?php echo $request->getRequestPath(), ($source?'">display demo</a>':'?source=TRUE">show-source</a>');?></strong></font>
   </td>
</tr>
<tr>
   <td bgcolor=#ffffff>
   ...
   $url = 'http://'.$request->getServerName().dirname($request->getScriptName()).'/mconfig.conf';
   ?>
   <dl><strong>Loading properties file from :</strong> 
   <dd><?php echo $url; ?>
   </dl>
   <?php
   $config = new MultiConfigLoader($url);
   $config->loadFromFile();
   ?>
   <dl>
